...
public class WaPrintMapping extends AllegroRecordSet {
	...
	private static final Resource FAUST_REGISTER_RESOURCE = new ClassPathResource("wa-print-mapping.txt", WaPrintMapping.class);

	private Map<String, Integer> registerToHierarchyMapping = new HashMap<String, Integer>();
	private Integer unregisteredFile;

	public static WaPrintMapping parse() throws IOException {
		WaPrintMapping wafr = new WaPrintMapping();
		wafr.parse(FAUST_REGISTER_RESOURCE);
		return wafr;
	}

	private WaPrintMapping() {
	}
