...
/**
 * Deploys the generated schema using <code>scp</code> or <code>file</code>
 * protocol to the site URL specified in the
 * <code>&lt;distributionManagement&gt;</code> section of the POM.
 * <p>
 * For <code>scp</code> protocol, the schema files are packaged into zip
 * archive, then the archive is transfered to the remote host, next it is
 * un-archived. This method of deployment should normally be much faster than
 * making a file by file copy. For <code>file</code> protocol, the files are
 * copied directly to the destination directory.
 * </p>
 * 
 * <p>
 * Copied from maven-site-plugin.
 * </p>
 * 
 ...
 * @phase deploy
 */
public class OddSchemaDeployMojo extends AbstractMojo implements Contextualizable {
	/**
	 ...
	 * Defaults to "-Rf".
	 * 
	 * @parameter expression="${maven.site.chmod.options}"
	 *            default-value="-Rf"
	 * @since 2.1
	 */
	...
	public void execute() throws MojoExecutionException {
		if (!inputDirectory.exists()) {
			throw new MojoExecutionException("The generated schema does not exist, please run the 'schema' goal first");
		}
		...
		getLog().debug("The schema will be deployed to '" + url + "'");
		...
		try {
			...
			throw new MojoExecutionException("Error uploading schema", e);
	...
	public static ProxyInfo getProxyInfo(Repository repository, WagonManager wagonManager) {
		...
		for (int i = 0; i < nonProxyHosts.length; i++) {
			...
			if (StringUtils.contains(nonProxyHost, "*")) {
				...
				// prefix*
				if (StringUtils.isNotEmpty(nonProxyHostPrefix) && host.startsWith(nonProxyHostPrefix) && StringUtils.isEmpty(nonProxyHostSuffix)) {
					return null;
				}
				// *suffix
				if (StringUtils.isEmpty(nonProxyHostPrefix) && StringUtils.isNotEmpty(nonProxyHostSuffix) && host.endsWith(nonProxyHostSuffix)) {
					return null;
				}
				// prefix*suffix
				if (StringUtils.isNotEmpty(nonProxyHostPrefix) && host.startsWith(nonProxyHostPrefix) && StringUtils.isNotEmpty(nonProxyHostSuffix)
