...
public class EncodedTextDocumentValidator implements InitializingBean {
	...
	public List<String> validate(EncodedTextDocument document) {
		...
		try {
			byte[] documentData = XmlUtil.serialize(document.getDom());
			XmlUtil.SERIALIZATION_TEMPLATES.newTransformer().transform(new StreamSource(new ByteArrayInputStream(documentData)), new SAXResult(validator.getContentHandler()));
			return errors;
