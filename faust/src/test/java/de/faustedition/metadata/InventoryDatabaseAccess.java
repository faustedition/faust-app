...
public class InventoryDatabaseAccess extends AbstractContextTest {
	@Autowired
	...
	public void callNumbersWellformed() {
		...
		Log.LOGGER.info(String.format("Sub-File-Secondary-Suffices: { %s }",
				StringUtils.join(subFileSecondarySuffixSet, ", ")));
	...
	public void recordsMissingInArchiveDatabase() {
		...
		for (AllegroRecord record : faustInventory) {
			...
			if (matchingRecords.isEmpty()) {
				...
				Log.LOGGER.info(String.format("%s ==> { %s }", callNumber,
						StringUtils.join(matchingCallNumbers, "; ")));
			}
		}
