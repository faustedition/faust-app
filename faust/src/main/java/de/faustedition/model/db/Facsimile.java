...
public class Facsimile implements Serializable {
	...
	@Override
	public int hashCode() {
		return (name == null || manuscript == null) ? super.hashCode() : new HashCodeBuilder().append(name).append(
				manuscript.getId()).toHashCode();
	}

	@SuppressWarnings("unchecked")
	public static List<Facsimile> find(Session session, Manuscript manuscript) {
		return session.createCriteria(Facsimile.class).addOrder(Order.asc("name")).createCriteria("manuscript").add(
				Restrictions.idEq(manuscript.getId())).list();
	}

	@SuppressWarnings("unchecked")
	public static Facsimile find(Session session, Manuscript manuscript, String name) {
		return DataAccessUtils.uniqueResult((List<Facsimile>) session.createCriteria(Facsimile.class).add(
				Restrictions.eq("name", name)).createCriteria("manuscript").add(
				Restrictions.idEq(manuscript.getId())).list());
	}
	...
	public static Facsimile findByImagePath(Session session, String imagePath) {
		return DataAccessUtils.singleResult((List<Facsimile>) session.createCriteria(Facsimile.class).add(
				Restrictions.eq("imagePath", imagePath)).list());
	}
