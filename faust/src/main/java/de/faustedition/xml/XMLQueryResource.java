...
public class XMLQueryResource {
    ...
    @Inject
    public XMLQueryResource(Sources sources, Templates templates) {
    ...
    private Templates.ViewAndModel query(String xpath, String folder, String queryMode) {
        ...
        if (!xpath.isEmpty()) {
            ...
            for (File xml : sources.directory(folder)) {
                Map<String, Object> entry = Maps.newHashMap();

                try {
                    entry.put("uri", new URI("faust", "xml", sources.path(xml), null).toString());
                    final Document document = XML.newDocumentBuilder().parse(xml);
