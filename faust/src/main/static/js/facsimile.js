...
Faust.YUI().use("dom", "node", "io", "dump", function(Y) {
	...
	Faust.FacsimileViewer = new Class({
	   	...
	   	*/
	    	initialize: function( main_id, options ) {
	    		this.server = Faust.FacsimileServer
	    		this.credit = "Digitale Faust-Edition";
				...
				this.images = [{ src:options['image'], sds:"0,90"} ];
			}
	  	...
	  	/* Refresh function to avoid the problem of tiles not loading properly in Firefox/Mozilla */
	  	refresh: function(){
	    		var unloaded = 0;
			...
			*/
	    		...
	    		/* If no tiles are missing, destroy our refresher timer, fade out our loading animation and and reset our cursor */
	    		if( unloaded == 0 ){
	      			$clear( this.refresher );
	      			this.refresher = null;
	    		}
		...
		/* Move to a particular position (via absolute coordinates, incorporating scale) */
		moveTo: function(x, y) {
			x = x * this.wid / this.max_width;
			y = y * this.hei / this.max_height;
			Y.log(Y.dump({ x: x, y: y}));
			this.scrollTo(x - this.rgn_x, y - this.rgn_y);
		},
		
	  	/* Scroll to a particular position (via delta) */
	  	...
	  	createWindows: function(){
	    		...
	    		if( this.targetclick ) {
				el.addEvent( 'click', this.targetclick.bindWithEvent(this) );
			}
	    		...
	    		new Element('img', {src: Faust.contextPath + '/static/img/facsimile/iip.32x32.png', id: 'info', styles: { opacity: 0.8 } } ).injectInside('logo');

	    		// Fix IE7 PNG transparency problem
	    		if( Browser.Engine.trident5 ){
	      			$('info').setStyle( 'filter', 'progid:DXImageTransform.Microsoft.AlphaImageLoader(enabled=true,src=' + Faust.contextPath + '"/static/img/facsimile/iip.32x32.png",sizingMethod=scale)' );
	    		}

	    		new Tips( '#info, #toolbar', {
				// We need this to force the tip in front of nav window
		  		className: 'tip', 
	  	...
	  	createNavigationWindow: function() {
				...
				id: 'navigation',
				src: this.server + '?FIF=' + this.images[0].src + '&SDS=' + this.images[0].sds + '&CNT=1.0' + '&WID=' + this.min_x + '&QLT=99&CVT=jpeg'
	    		});
	   		...
	   		var loadBarContainer = new Element('div', {
				...
				id: 'navbuttons',
				html: 
					'<img id="shiftLeft" src="' + Faust.contextPath + '/static/img/facsimile/left.png"/>' +
					'<img id="shiftUp" src="' + Faust.contextPath + '/static/img/facsimile/up.png"/>' +
					'<img id="shiftRight" src="' + Faust.contextPath + '/static/img/facsimile/right.png"/><br/>' +
					'<img id="shiftDown" src="' + Faust.contextPath + '/static/img/facsimile/down.png"/><br/>' +
					'<img id="zoomIn" src="' + Faust.contextPath + '/static/img/facsimile/zoomIn.png"/>' +
					'<img id="zoomOut" src="' + Faust.contextPath + '/static/img/facsimile/zoomOut.png"/>' +
					'<img id="reset" src="' + Faust.contextPath + '/static/img/facsimile/reset.png"/>'
	    			});
	  	...
	  	setScale: function() {
	    		...
	    		$('scale').set({ styles: {width: pixels}, html: label });
	  	},

	  	/* Use a AJAX request to get the image size, tile size and number of resolutions from the server */
	  	load: function(){
			var fv = this;
			Faust.YUI().use("io-xdr", function(Y) {
				Y.io.transport({ id: "flash", src: Faust.contextPath + '/static/yui3/build/io/io.swf' });
				Y.on('io:xdrReady', function() {
					Y.io(Faust.FacsimileServer, {
						xdr: { use: 'flash', responseXML: false },
					  	data: "FIF=" + Faust.encodePath(fv.images[0].src) + "&obj=IIP,1.0&obj=Max-size&obj=Tile-size&obj=Resolution-number",
					  	timeout: 3000,
						on: {
							success: function(id, o, args) {
								var response = o.responseText;
								var tmp = response.split( "Max-size" );
					    			if(!tmp[1]) alert( "Unexpected response from server " + Faust.FacsimileServer);
					    			var size = tmp[1].split(" ");
					    			fv.max_width = parseInt( size[0].substring(1,size[0].length) );
					    			fv.max_height = parseInt( size[1] );
					    			tmp = response.split( "Tile-size" );
					    			size = tmp[1].split(" ");
					    			fv.tileSize[0] = parseInt( size[0].substring(1,size[0].length) );
					    			fv.tileSize[1] = parseInt( size[1] );
					    			tmp = response.split( "Resolution-number" );
					    			fv.num_resolutions = parseInt( tmp[1].substring(1,tmp[1].length) );
					    			fv.res = fv.num_resolutions;
					    			fv.createWindows();
							},
